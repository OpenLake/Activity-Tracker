...
const psapi = new ffi.Library('psapi', {
	...
	GetProcessMemoryInfo: [
		'int',
		['pointer', ProcessMemoryCountersPointer, 'uint32'],
	],
});
...
function windows() {
	...
	const windowTextBufferClean = ref.reinterpretUntilZeros(
		windowTextBuffer,
		wchar.size,
	);
	...
	const getProcessMemoryInfoResult = psapi.GetProcessMemoryInfo(
		processHandle,
		memoryCounters.ref(),
		ProcessMemoryCounters.size,
	);
	...
	const getWindowRectResult = user32.GetWindowRect(
		activeWindowHandle,
		bounds.ref(),
	);
